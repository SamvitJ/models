...
def inference(images, fft=False):
  ...
  with tf.variable_scope('conv1') as scope:
    kernel = _variable_with_weight_decay('weights-' + str(fft),
                                         shape=[5, 5, 3, 64],
                                         stddev=5e-2,
                                         wd=0.0)
    ...
    biases = _variable_on_cpu('biases-' + str(fft), [64], tf.constant_initializer(0.0))
  ...
  with tf.variable_scope('conv2') as scope:
    ...
    biases = _variable_on_cpu('biases-' + str(fft), [64], tf.constant_initializer(0.1))
    pre_activation = tf.nn.bias_add(conv, biases)
  ...
  with tf.variable_scope('local3') as scope:
    ...
    weights = _variable_with_weight_decay('weights-' + str(fft), shape=[dim, 384],
                                          stddev=0.04, wd=0.004)
    biases = _variable_on_cpu('biases-' + str(fft), [384], tf.constant_initializer(0.1))
    local3 = tf.nn.relu(tf.matmul(reshape, weights) + biases, name=scope.name)
    _activation_summary(local3)

  # local4
  with tf.variable_scope('local4') as scope:
    weights = _variable_with_weight_decay('weights-' + str(fft), shape=[384, 192],
                                          stddev=0.04, wd=0.004)
    biases = _variable_on_cpu('biases-' + str(fft), [192], tf.constant_initializer(0.1))
  ...
  with tf.variable_scope('softmax_linear') as scope:
    weights = _variable_with_weight_decay('weights-' + str(fft), [192, NUM_CLASSES],
                                          stddev=1/192.0, wd=0.0)
    biases = _variable_on_cpu('biases-' + str(fft), [NUM_CLASSES],
